...
layout (location = 0) in vec3 vertex_position; // attribute location number = 0, describes the vertex position (x, y, z)
...
void main() {
    ...
    gl_Position = proj_matrix * view_matrix * model_matrix * vec4(vertex_position, 1.0); // initialize the gl_Position (just some vertex shader output variable)
